...
# ============================================================
# SHIJO.AI FINAL FIX SCRIPT (NULL-SAFE + UUID COMPATIBLE)
# Fixes tsconfig.json and all route imports with proper null handling
# ============================================================

set -e

echo "🔧 Starting complete fix with null-safe quota checking..."
...
echo "📝 Fixing tsconfig.json with proper path aliases..."
...
# ============================================================
# STEP 2: FIX ALL ROUTE FILES (NULL-SAFE)
# ============================================================

echo "🔧 Fixing all route files with null-safe quota checking..."

# Fix classify route - UUID compatible
...
# Fix expand route - NULL-SAFE quota checking
...
export async function POST(
  ...
  try {
    ...
    // Check quota with null-safe defaults
    ...
    if (!quota) {
      return NextResponse.json(
        { error: 'User quota not found', quotaExceeded: true },
        { status: 429 }
      );
    }
    
    // Null-safe quota checking with defaults
...
if [ $? -eq 0 ]; then
  ...
  git add .
  git commit -m "fix: resolve tsconfig, UUID handling, and null-safe quota checking"
