...
1. Push these 3 fixes (self-healing Stripe customer check ×2 files, dashboard tool-count fix, tools-list model badge fix) — see git commands in §21.
2. Grab the Vercel Runtime Log line for the `/api/account/export` 500 from just now, so the real error can be found and fixed (Bug 4).
3. After push: retry "Upgrade to Pro" on `srikanth@shiroapps.com` to confirm the Stripe self-heal actually works and reaches a real Checkout page (can screenshot the Checkout page itself to also finally confirm test vs. live mode, without completing a purchase).
4. Optional: flip `is_admin = true` for `srikanth@shiroapps.com` via Neon SQL Editor (same low-risk pattern as the earlier `reason` column migration) so `/admin/users` and `/admin/tickets` can be verified end-to-end with real data instead of just the access-denied gate.
