...
# ============================================================
# SHIJO.AI COMPLETE FIX SCRIPT (UUID COMPATIBLE)
# Fixes tsconfig.json and all route imports with proper UUID handling
# ============================================================

set -e

echo "🔧 Starting complete fix with UUID support..."
...
# ============================================================
# STEP 2: FIX ALL ROUTE FILES (UUID COMPATIBLE)
# ============================================================

echo "🔧 Fixing all route files with UUID support..."
...
# Fix expand route - UUID compatible
...
export async function POST(
  ...
  try {
    ...
    if (!quota || quota.expansionsUsed >= quota.expansionsQuota) {
      return NextResponse.json(
    ...
    await db
      .update(userQuotas)
      .set({
        expansionsUsed: quota.expansionsUsed + 1,
        updatedAt: new Date(),
      })
      .where(eq(userQuotas.userId, userId));
    ...
    return NextResponse.json({
      expansions,
      quotaRemaining: quota.expansionsQuota - quota.expansionsUsed - 1,
    });
...
if [ $? -eq 0 ]; then
  ...
  git add .
  git commit -m "fix: resolve tsconfig path aliases and UUID type handling in routes"
